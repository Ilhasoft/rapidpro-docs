...
`@results` → The results for the current run.
`@fields` → The custom field for the contact.
`@input` → The most rescent input.
`@urns` → The URNs for the contact.
`@run` → A run in this flow.
`@child` → Run details collected in a child flow, if any.
`@parent` → Run details collected in a parent flow, if any.
`@webhook` → The parsed JSON payload of the last webhook call.
`@trigger` → A trigger that initiated a session.
